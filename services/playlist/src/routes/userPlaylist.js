...
router.post("/deletesong", async (req, res) => {
    try {
        ...
        await playlist.updateOne({
            songs: await playlist.songs.filter(item => item._id.toString() !== songId.toString())
        })
...
router.post("/updatename", async (req, res) => {
    ...
    try {
        ...
        await user.updateOne({
            myPlaylists: [...await user.myPlaylists.filter(item => item.playlistId !== playlistId), {
                name,
                playlistId
            }]
        });
...
router.get("/delete/:id", async (req, res) => {
    try {
        ...
        await user.updateOne({
            myPlaylists: await user.myPlaylists.filter(item => item.playlistId !== _id)
        });
