...
import { coreActions, addArtistActions } from "../actions";
...
class Artists extends Component {
	...
	async createArtist() {
		try {
			const data = await this.props.toggleAddArtistDialog(true, false, null, "", null);
			if (data["status"]) {
	...
	async editArtist(index) {
		try {
			...
			const data = await this.props.toggleAddArtistDialog(true, true, editArtistId, artistName, artistUrl);
			if (data["status"]) {
				await this.fetchArtists();
...
const mapDispatchToProps = dispatch => {
	return {
		...
		toggleAddArtistDialog: (isOpened, isEdit, artistId, artistName, artistImageUrl) => {
			return addArtistActions.toggleAddArtistDialog(isOpened, isEdit, artistId, artistName, artistImageUrl);
		},
	};
};
