...
class Albums extends Component {
    ...
    fetchPopularAlbumsHandler = async (fetchFull = false) => {
        ...
        if (fetchFull)
            data = await this.props.fetchPopularAlbums(1, 10000);
        else
            data = await this.props.fetchPopularAlbums(1, 10);

        if (this._isMounted)
            this.setState({ isLoading: false, popularAlbums: data })
    }
    ...
    fetchLatestAlbumsHandler = async (fetchFull = false) => {
        ...
        if (fetchFull)
            data = await this.props.fetchLatestAlbums(1, 10000);
        else
            data = await this.props.fetchLatestAlbums(1, 10);

        if (this._isMounted)
            this.setState({ isLoading: false, latestAlbums: data })
    }
    ...
    componentDidUpdate(nProps) {
        if (nProps.match.params.type !== this.props.match.params.type)
            this.albumsMainHandler();
    }
    ...
    render() {
        return this.state.isLoading ?
            ...
            <div className="albums-wrapper">
                {this.state.type === 'all' ? <Fragment> < this.LatestAlbums />
                    <this.PopularAlbums /></Fragment> : <this.AllAlbums />
                }
            </div>
    }
}
