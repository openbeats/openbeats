...
export default class SongSearcher extends Component {
    ...
    fetchSearchStringSuggestion = async () => {
        if (this.state.searchStringSuggestionFetchUrl && this.state.suggestionString.length) {
            ...
            let suggestionData = (await axios.get(this.state.searchStringSuggestionFetchUrl + this.state.suggestionString)).data;
            this.setState({ suggestionStringArray: [this.state.suggestionString, ...suggestionData.data.slice(0, 5)], suggestionCurrentIndex: 0 })
    ...
    fetchSongSuggestion = async () => {
        ...
        if (this.state.songSuggestionFetchUrl) {
            let suggestionData = (await axios.get(this.state.songSuggestionFetchUrl + this.state.suggestionString)).data;
            this.setState({ songsCollection: suggestionData.data, suggestionStringArray: [] })
        }
    }
    ...
    clearSuggestionInputListener = (event) => {
        if (event.keyCode === 27 || !this.suggestionBlockRef.contains(event.target)) {
            this.setState({ suggestionStringArray: [], suggestionString: "", suggestionCurrentIndex: 0 })
            document.removeEventListener("click", this.clearSuggestionInputListener);
            document.removeEventListener("keyup", this.clearSuggestionInputListener);
        } else if (event.keyCode === 40) {
            this.setState({ suggestionCurrentIndex: (this.state.suggestionCurrentIndex + 1) < this.state.suggestionStringArray.length ? (this.state.suggestionCurrentIndex + 1) : 0 })
        } else if (event.keyCode === 38) {
            this.setState({ suggestionCurrentIndex: (this.state.suggestionCurrentIndex - 1) >= 0 ? this.state.suggestionCurrentIndex - 1 : this.state.suggestionStringArray.length - 1 })
        ...
        return (
            <div className="song-searcher-wrapper">
                <div className="song-searcher-header">
                    <div className="song-searcher-header-input" ref={d => this.suggestionBlockRef = d}>
                        <input type="text" ref={d => this.inputSearchRef = d} onChange={async (e) => {
                ...
                <div className="song-searcher-body">
                    {this.state.songsCollection.length === 0 && <div className="song-searcher-empty-result-message font-weight-bold" onClick={() => { this.inputSearchRef.focus() }}>Search Songs Here!</div>}
                            ...
                            {this.state.songsCollection.map((item, key) => (
                                <div className="song-searcher-search-result-node" key={key}>
                                    <div className="song-searcher-song-thumbnail-holder">
                                        <div className="song-searcher-song-thumbnail" style={{ backgroundImage: `url(${item.thumbnail})` }}></div>
                                    </div>
                                    <div className="song-searcher-actions ml-3">
                                        ...
                                        <i className="fas fa-plus-circle shadow cursor-pointer ml-4" onClick={() => this.addSongToBucket(key)}></i>
                                    </div>
